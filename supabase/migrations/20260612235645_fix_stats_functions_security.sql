...
/*
# Fix stats functions security

1. Security Changes
- Switch `get_stats_globales` from SECURITY DEFINER to SECURITY INVOKER
- Switch `get_stats_par_caisse` from SECURITY DEFINER to SECURITY INVOKER
- Set immutable `search_path = ''` on both functions
- All table references already use fully-qualified `public.` schema prefix
- Retain EXECUTE grant to anon (needed for pin-based custom auth)

2. Notes
- SECURITY INVOKER means the functions run with the caller's privileges
- RLS policies on underlying tables still apply, restricting data access
- Fixed search_path prevents search-path hijacking attacks
*/
