...
export default function LoginPage({ onLogin }: Props) {
  ...
  return (
    <div
      ...
      className="min-h-screen bg-gray-50 flex items-center justify-center p-4 outline-none"
    >
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500 mt-1 text-sm">Gestion de caisse simplifiée</p>
        </div>
            ...
            {Array.from({ length: PIN_LENGTH }).map((_, i) => (
              <div
                ...
                className={`w-14 h-14 rounded-2xl border-2 flex items-center justify-center transition-all duration-200 ${
                  i < digits.length
                    ? 'bg-emerald-500 border-emerald-500 scale-105 shadow-md shadow-emerald-200'
                    : i === digits.length
        ...
        <p className="text-center text-xs text-gray-400 mt-6">
          Tapez votre code à 4 chiffres ou utilisez le clavier
        </p>
      </div>
    </div>
  );
