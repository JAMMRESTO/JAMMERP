...
export function CategoryBar({ categories, selectedId, onSelect }: CategoryBarProps) {
  return (
    <div className="flex flex-wrap gap-1.5 sm:gap-2 pb-0.5 sm:pb-1 flex-shrink-0">
      {/* All */}
      <motion.button
        onClick={() => onSelect(null)}
        whileTap={{ scale: 0.95 }}
        className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl border text-xs sm:text-sm font-medium transition-all
          ${selectedId === null
