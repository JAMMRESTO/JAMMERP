...
export function Dashboard() {
  ...
  const load = useCallback(async () => {
    ...
    const [todaySales, yestSales, weekSales, saleItems, orders, lowProducts, lowIngredients, salesByUser, allUsers] = await Promise.all([
      ...
      sf(supabase.from('orders').select('id, order_number, status, order_type, customer_name, created_at')).in('status',['pending','preparing','ready']).order('created_at',{ ascending: false }).limit(5),
    ...
    // ─── Live orders ───────────────────────────────────────────
    setLiveOrders((orders.data ?? []).map((o: {id: string; order_number: number; status: string; order_type: string; customer_name: string; created_at: string}) => ({
      id: o.id, order_number: o.order_number,
      table: o.customer_name || TYPE_LABELS[o.order_type] || 'N/A',
