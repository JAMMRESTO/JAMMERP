...
import {
  ...
  ArrowLeft, Clock, Search, FileText, CreditCard, Locate, Navigation
} from 'lucide-react';
import { supabase } from '../lib/supabase';
import type { Product, Category, OnlineOrderType, OnlineOrderItem, OnlineOrderPayment } from '../types/database';
...
const PAYMENT_OPTIONS: { id: OnlineOrderPayment; label: string; desc: string }[] = [
  { id: 'cash', label: 'Especes', desc: 'Payer a la livraison' },
  { id: 'wave', label: 'Wave', desc: 'Paiement mobile Wave' },
  { id: 'orange_money', label: 'Orange Money', desc: 'Paiement mobile OM' },
  { id: 'card', label: 'Carte', desc: 'Carte bancaire' },
];
...
function CartDrawer({ entries, sym, taxRate, onClose, onCheckout, onAdd, onRemove }: {
...
interface CheckoutForm {
  ...
  notes: string;
  payment_method: OnlineOrderPayment;
  lat: number | null;
  lng: number | null;
}
...
function CheckoutStep({ entries, sym, taxRate, onBack, onConfirm, loading }: {
  entries: CartEntry[]; sym: string; taxRate: number;
  onBack: () => void; onConfirm: (form: CheckoutForm) => void;
  loading: boolean;
}) {
  const [form, setForm] = useState<CheckoutForm>({
    name: '', phone: '', address: '', type: 'delivery',
    notes: '', payment_method: 'cash', lat: null, lng: null,
  });
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="flex-1 max-w-lg mx-auto w-full px-4 py-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
          <p className="font-semibold text-gray-700 text-sm mb-3">Mode de recuperation</p>
          <div className="grid grid-cols-2 gap-2">
            {([
              { id: 'delivery', label: 'Livraison', icon: Truck, desc: 'Livre a votre adresse' },
              { id: 'takeaway', label: 'A emporter', icon: Package, desc: 'Recuperation sur place' },
            ] as const).map(t => {
            ...
            <>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={geoLoading}
                  className="flex items-center gap-2 px-3 py-2 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl text-blue-700 text-xs font-medium transition-colors disabled:opacity-50"
                >
                  {geoLoading ? <Loader2 size={13} className="animate-spin" /> : <Locate size={13} />}
                  {form.lat ? 'Position mise a jour' : 'Partager ma position'}
                </button>
                {form.lat && (
                ...
                {geoError && <span className="text-red-500 text-[10px]">{geoError}</span>}
              </div>
            </>
          )}
          <div className="relative">
            ...
            <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
              placeholder="Instructions particulieres (optionnel)" rows={2}
              className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-orange-400 resize-none transition-colors" />
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {PAYMENT_OPTIONS.map(opt => {
              ...
              return (
                <button key={opt.id} onClick={() => setForm(f => ({ ...f, payment_method: opt.id }))}
                  className={`flex flex-col items-start gap-0.5 p-3 rounded-xl border-2 text-left transition-all ${active ? 'border-orange-400 bg-orange-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                  <span className={`font-semibold text-xs ${active ? 'text-orange-700' : 'text-gray-700'}`}>{opt.label}</span>
        ...
        {/* Order summary */}
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
          <p className="font-semibold text-gray-700 text-sm mb-3">Recapitulatif</p>
          <div className="space-y-2 mb-3">
            {entries.map(e => (
              <div key={e.product.id} className="flex justify-between text-sm">
                <span className="text-gray-600">{e.quantity}x {e.product.name}</span>
          ...
          <div className="border-t border-gray-100 pt-2.5 space-y-1">
            ...
            <div className="flex justify-between text-gray-800 font-bold text-sm pt-1.5 border-t border-gray-100">
              <span>Total a payer</span><span className="text-orange-600">{fmt(total, sym)}</span>
            </div>
          </div>
        </div>
...
function ConfirmedScreen({ orderId, orderNumber, orderType, restName }: {
  orderId: string; orderNumber: number; orderType: OnlineOrderType; restName: string;
}) {
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 text-center">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <h2 className="text-2xl font-black text-gray-800 mb-1">Commande confirmee !</h2>
        <p className="text-gray-500 text-base mb-4">
          Commande <span className="font-bold text-gray-700">#{String(orderNumber).padStart(4, '0')}</span> recue par <span className="font-bold">{restName}</span>
        </p>
        <div className="flex items-center justify-center gap-2 mb-6 bg-orange-50 border border-orange-200 rounded-2xl px-5 py-3">
          {orderType === 'delivery'
            ? <><Truck size={18} className="text-orange-500" /><span className="text-orange-700 font-semibold text-sm">Livraison en cours de preparation</span></>
            : <><Package size={18} className="text-orange-500" /><span className="text-orange-700 font-semibold text-sm">Votre commande est en preparation</span></>
          }
        </div>

        <a
          href={`/order/track?id=${orderId}`}
          className="inline-flex items-center justify-center gap-2 px-8 py-3 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-2xl shadow-md shadow-orange-500/25 transition-colors text-sm"
        >
          <Navigation size={16} />
          Suivre ma commande en temps reel
        </a>

        <div className="flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3 max-w-xs mx-auto mt-5 text-left">
          <Clock size={15} className="text-blue-500 flex-shrink-0 mt-0.5" />
          <p className="text-blue-700 text-sm">Vous serez contacte pour confirmer les details de votre commande.</p>
        </div>
      </motion.div>
    </div>
  );
...
export function OrderPage() {
  ...
  const [confirmedOrder, setConfirmedOrder] = useState<{ id: string; number: number; type: OnlineOrderType } | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [settingsRes, catsRes, prodsRes] = await Promise.all([
        settingsQuery, catsQuery, prodsQuery,
      ]);
  ...
  async function handleConfirm(form: CheckoutForm) {
    setSubmitting(true);
    ...
    const { data } = await supabase.from('online_orders').insert({
      ...
      customer_lat: form.lat,
      customer_lng: form.lng,
      items,
      subtotal,
      tax_amount: tax,
      discount_amount: 0,
    ...
    setSubmitting(false);
    if (data) {
      setConfirmedOrder({ id: data.id, number: data.order_number, type: form.type });
      setStep('confirmed');
      setCart([]);
    }
  }
      ...
      <div className="flex-1 px-4 py-5 pb-28 max-w-2xl mx-auto w-full">
        {filteredProducts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-gray-400 font-medium">Aucun article trouve</p>
          </div>
        ) : (
